...
**IMPORTANT — pssCompliance in K8sAnalysisResult:** Check if `K8sAnalysisResult` in types.ts already includes a `pssCompliance` field. Looking at the types: `K8sAnalysisResult` has `violations`, `score`, `resources`, `resourceSummary`, `parseSuccess`, `timestamp` — it does NOT include `pssCompliance`. However, `K8sEngineResult` does. Two options:
- Option A: Add `pssCompliance?: PssComplianceSummary` to `K8sAnalysisResult` in types.ts
- Option B: Compute it from violations in the results panel

Go with **Option A** — add the optional `pssCompliance` field to `K8sAnalysisResult` in types.ts since the engine already computes it and it's wasteful to recompute. Update the type AND ensure Plan 02's K8sEditorPanel passes `pssCompliance: engineResult.pssCompliance` in the k8sResult.set() call.
...
**types.ts** — Add `pssCompliance` field to `K8sAnalysisResult`:
- Add `pssCompliance?: PssComplianceSummary;` to the `K8sAnalysisResult` interface (after `resourceSummary`)
- This allows the results panel to display PSS compliance data that the engine already computes

Then verify that `K8sEditorPanel.tsx` from Plan 02 passes `pssCompliance: engineResult.pssCompliance` in the `k8sResult.set()` call. If it doesn't (because types.ts didn't have the field when Plan 02 ran), add it:
- In the k8sResult.set() call's object literal, add `pssCompliance: engineResult.pssCompliance` alongside the existing `resources`, `resourceSummary`, etc.
  </action>
  <verify>
...
8. K8sAnalysisResult type includes optional pssCompliance field
  </verify>
  <done>
