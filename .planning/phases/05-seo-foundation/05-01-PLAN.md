...
4. Create `src/pages/rss.xml.ts`:
  ...
  <verify>
    Run `npm run build` — should complete without errors. Check dist/rss.xml exists and contains blog post entries. Check dist/sitemap-index.xml exists and references a sitemap file. Verify robots.txt references the sitemap: `grep 'sitemap-index.xml' public/robots.txt` should return the Sitemap line (already created in Phase 1).
  </verify>
  <done>
    SEOHead component created with OG, Twitter, canonical tags. RSS feed generates at /rss.xml with blog posts. Sitemap generates at /sitemap-index.xml. robots.txt confirmed to reference sitemap-index.xml. Build succeeds.
  </done>
</task>
...
7. `grep 'sitemap-index.xml' public/robots.txt` confirms robots.txt references the sitemap
</verification>
...
- robots.txt at site root points to sitemap-index.xml (confirmed via grep)
