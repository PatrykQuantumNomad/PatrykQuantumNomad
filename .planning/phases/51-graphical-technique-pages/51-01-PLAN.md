...
**Code style:** Follow project conventions -- 2-space indent, single quotes, camelCase functions, SCREAMING_SNAKE for const maps, named exports, explicit return types.
  ...
  <verify>
    <automated>cd /Users/patrykattc/work/git/PatrykQuantumNomad && npx astro check 2>&1 | tail -10 && node -e "const fs=require('fs'); const f=fs.readFileSync('src/lib/eda/technique-renderer.ts','utf8'); const entries=(f.match(/^\s+'[\w-]+'\s*:/gm)||[]); console.log('TECHNIQUE_RENDERERS entries:', entries.length); console.log(entries.length>=29 ? 'PASS: 29+ slug entries' : 'FAIL: only ' + entries.length + ' entries, need 29');"</automated>
...
6. All 29 graphical slugs from techniques.json have entries in TECHNIQUE_RENDERERS (confirmed by automated entry count check)
</verification>
...
- Automated verify confirms >= 29 entries in TECHNIQUE_RENDERERS map
</success_criteria>
