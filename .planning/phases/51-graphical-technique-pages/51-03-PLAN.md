...
must_haves:
  truths:
    ...
    - "BreadcrumbJsonLd structured data uses the actual technique slug, not a title-derived slug"
    - "Thumbnail generator utility exists and produces sized SVG strings for any technique slug (wiring to landing page cards is Phase 54)"
  artifacts:
    ...
    - path: "src/lib/eda/thumbnail.ts"
      provides: "Build-time SVG thumbnail generation utility for technique cards"
      exports: ["generateThumbnail"]
    - path: "src/components/eda/TechniquePage.astro"
      provides: "Updated technique page layout with slug prop, category-aware related links, and correct BreadcrumbJsonLd URLs"
      contains: "slug"
  key_links:
    ...
    - from: "src/pages/eda/techniques/[slug].astro"
      ...
      via: "layout wrapper with named slots, passing slug prop"
    ...
    - from: "src/components/eda/TechniquePage.astro"
      to: "BreadcrumbJsonLd"
      via: "slug prop used for structured data URL instead of title-derived slug"
      pattern: "slug.*techniques"
...
<task type="auto">
  <name>Task 1: Fix TechniquePage.astro (slug prop, BreadcrumbJsonLd, related links) and create [slug].astro dynamic route</name>
  <files>src/components/eda/TechniquePage.astro, src/pages/eda/techniques/[slug].astro</files>
  <action>
**Part A: Fix TechniquePage.astro — slug prop, BreadcrumbJsonLd bug, and related technique links**

The current TechniquePage.astro has two issues:
1. Related technique links hardcode `/eda/techniques/{slug}/` which is wrong for cross-category references
2. BreadcrumbJsonLd on line 25 derives the URL from `title.toLowerCase().replace(/\s+/g, '-')` instead of using the actual technique slug — this produces wrong URLs for techniques like "Box-Cox Linearity Plot" (would generate `box-cox-linearity-plot` instead of actual slug `box-cox-linearity`)

Update the component:

1. Add `slug?: string` to the Props interface:
```typescript
interface Props {
  title: string;
  description: string;
...
2. Destructure `slug` in the frontmatter:
```typescript
const { title, description, category, nistSection, slug, relatedTechniques = [], useKatex = false } = Astro.props;
```

3. Fix BreadcrumbJsonLd URL derivation (line 25) — use actual slug when provided, fall back to title-derived slug for backwards compatibility:
```astro
{ name: title, url: `https://patrykgolabek.dev/eda/techniques/${slug ?? title.toLowerCase().replace(/\s+/g, '-')}/` },
```

4. Change the related techniques link from:
```astro
...
This way the route file passes pre-computed URLs using `techniqueUrl()` from routes.ts, which handles graphical vs. quantitative routing correctly. The `url` field is now part of the relatedTechniques prop type.
...
**Template — pass `slug={technique.slug}` to TechniquePage for correct BreadcrumbJsonLd:**
```astro
<TechniquePage
...
- `slug={technique.slug}` is passed to TechniquePage so BreadcrumbJsonLd uses the correct slug
...
- Route file should be under ~120 lines total -- all heavy logic is in technique-renderer.ts and technique-content.ts
  ...
  <verify>
    ...
    <manual>Check that build output shows 29 new pages under /eda/techniques/ and total page count increased by 29. Also verify TechniquePage.astro contains `slug` prop and BreadcrumbJsonLd uses `slug ??` fallback pattern.</manual>
  </verify>
  <done>All 29 graphical technique pages build successfully at /eda/techniques/{slug}/, with Tier A pages showing static SVG plots and Tier B pages showing PlotVariantSwap with tab-based variant switching. Related technique links use category-aware URLs. BreadcrumbJsonLd structured data uses the actual technique slug (not title-derived). Build completes without errors.</done>
</task>

<task type="auto">
  <name>Task 2: Create thumbnail.ts utility for build-time SVG thumbnails</name>
  <files>src/lib/eda/thumbnail.ts</files>
  <action>
Create `src/lib/eda/thumbnail.ts` that generates simplified, smaller SVG thumbnails for technique cards. This is the LAND-05 thumbnail generator utility -- the wiring to actual landing page cards happens in Phase 54.
...
Choose the simplest approach that produces clean, small thumbnails. The landing page (Phase 54) will call `generateThumbnail(slug)` when rendering technique cards.
...
**Code style:** 2-space indent, single quotes, named exports.
  ...
  <done>thumbnail.ts exports generateThumbnail(slug) that produces accessibility-annotated SVG thumbnails sized for technique cards, reusing existing SVG generator output with scaled dimensions. Utility is ready for Phase 54 landing page integration (LAND-05).</done>
</task>
...
7. BreadcrumbJsonLd structured data URLs use the actual technique slug, not title-derived slug
8. Every page has prose sections (What It Is, When to Use It, How to Interpret, Assumptions)
9. thumbnail.ts exports generateThumbnail and produces SVG strings
</verification>
...
- BreadcrumbJsonLd structured data uses `slug` prop (not title-derived), producing correct URLs for all techniques
- Build-time SVG thumbnail utility exists via thumbnail.ts for Phase 54 landing page integration (LAND-05)
