...
3. **Replace block-plot** in TECHNIQUE_RENDERERS:
   ...
   - Replace with `generateBlockPlot` using data from doeFactors. Build blocks from the dataset:
     ```typescript
     'block-plot': () => {
       // Block by Catalyst (A/B), group by Temperature (Low/High)
       const blocks = ['A', 'B'].map(level => ({
         label: `Catalyst ${level}`,
         values: ['Low', 'High'].map(tLevel => ({
           group: `Temp ${tLevel}`,
           mean: mean(doeFactors.filter(f => f.factor === 'Catalyst' && f.level === level).filter(f => {
             // Cross-reference Temperature factor at same response indices
             return true; // Use simplified: split doeFactors by Catalyst level, compute mean
           }).map(f => f.response)),
         })),
       }));
       // Simplification: Use existing computed means from the current composeBlockPlot
       return generateBlockPlot({
         blocks: [
           { label: 'Block A', values: [
             { group: 'Catalyst', mean: mean(doeFactors.filter(f => f.factor === 'Catalyst' && f.level === 'A').map(f => f.response)) },
             { group: 'Temperature', mean: mean(doeFactors.filter(f => f.factor === 'Temperature' && f.level === 'Low').map(f => f.response)) },
           ]},
           { label: 'Block B', values: [
             { group: 'Catalyst', mean: mean(doeFactors.filter(f => f.factor === 'Catalyst' && f.level === 'B').map(f => f.response)) },
             { group: 'Temperature', mean: mean(doeFactors.filter(f => f.factor === 'Temperature' && f.level === 'High').map(f => f.response)) },
           ]},
         ],
         title: 'Block Plot',
     ...
     ```
     Note: The exact data wiring should preserve the same DOE factor data currently used by `composeBlockPlot` but pass it into the `generateBlockPlot` API format. The key output difference is connected dots instead of bars.
