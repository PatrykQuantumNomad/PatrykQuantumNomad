...
- KA-C012 (writable filesystem / read-only FS not enforced) -> No direct CIS section (omit)
...
- KA-C015 (Docker socket mount) -> 5.2.12 "Minimize the admission of HostPath volumes"
- KA-C016 (SA token auto-mount) -> 5.1.6 "Ensure that Service Account Tokens are only mounted where necessary"
- KA-C017 (default ServiceAccount) -> 5.1.5 "Ensure that default service accounts are not actively used"
