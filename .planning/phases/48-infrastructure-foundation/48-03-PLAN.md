...
@.planning/phases/48-infrastructure-foundation/48-02-SUMMARY.md
...
7. After build, verify D3 isolation using stats.html (which retains original module names even after Rollup minification):
...
8. After verification, remove visualizer from astro.config.mjs (revert the vite.plugins addition). Keep the npm package installed for future INFRA-11 re-verification.
  ...
  <verify>
    <automated>cd /Users/patrykattc/work/git/PatrykQuantumNomad && npm run build 2>&1 | tail -10 && echo "--- D3 module count in stats.html ---" && grep -c '"d3-selection"' dist/stats.html && echo "--- KaTeX test page D3 check ---" && grep -c "d3" dist/eda/test-formulas/index.html 2>/dev/null || echo "0"</automated>
    <manual>Build succeeds. stats.html contains d3-selection references (proving D3 was bundled). The KaTeX test page (test-formulas) has 0 references to D3 chunks. stats.html treemap confirms D3 modules are isolated to the distribution stub chunk only.</manual>
  </verify>
  <done>D3 micro-modules installed (6 runtime + 6 type packages). Bundle analysis via stats.html confirms D3 code appears in exactly 1 JS chunk loaded only by the distribution test page. Non-distribution EDA pages load zero D3 code. Visualizer removed from production config after verification.</done>
</task>
