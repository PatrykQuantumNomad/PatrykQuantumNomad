...
must_haves:
  truths:
    ...
    - "OG cache utility computes deterministic hashes and exposes getOrGenerateOgImage for Phase 55 endpoints"
  ...
  artifacts:
    ...
    - path: "src/lib/eda/og-cache.ts"
      ...
      exports: ["computeOgHash", "getCachedOgImage", "cacheOgImage", "getOrGenerateOgImage"]
...
IMPORTANT: This utility runs at build time only (Node.js APIs). It is NOT a client-side module.
  ...
  <verify>
    <automated>cd /Users/patrykattc/work/git/PatrykQuantumNomad && npx tsx -e "import { computeOgHash, getOrGenerateOgImage } from './src/lib/eda/og-cache.ts'; const h1 = computeOgHash('test', 'desc'); const h2 = computeOgHash('test', 'desc'); const h3 = computeOgHash('other', 'desc'); console.log('hash length:', h1.length); console.log('deterministic:', h1 === h2); console.log('unique:', h1 !== h3); console.log('getOrGenerateOgImage exported:', typeof getOrGenerateOgImage === 'function');"</automated>
    <manual>computeOgHash returns a 12-character hex string. Same inputs produce same output (deterministic). Different inputs produce different output. getOrGenerateOgImage is exported as a function.</manual>
  </verify>
  <done>OG cache utility exports computeOgHash, getCachedOgImage, cacheOgImage, and getOrGenerateOgImage. Hash is deterministic (same title+description = same hash). Cache version included in hash for invalidation. getOrGenerateOgImage is ready for Phase 55 OG image endpoints. .gitignore updated to exclude cached PNGs.</done>
</task>
...
- OG cache utility produces deterministic hashes with version salt and exposes getOrGenerateOgImage for Phase 55
