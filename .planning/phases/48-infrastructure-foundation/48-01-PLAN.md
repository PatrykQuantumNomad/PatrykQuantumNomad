...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/pages/eda/test-formulas.astro"
      provides: "KaTeX validation page with 10+ formula categories"
      contains: "\\sum_{i=1}"
  ...
  key_links:
    ...
    - from: "src/pages/eda/test-formulas.astro"
      to: "src/layouts/EDALayout.astro"
      via: "layout import with useKatex=true"
...
4. Create `src/pages/eda/test-formulas.astro`:
   - Import EDALayout (NOT Layout.astro)
   - Set `useKatex={true}`
   - Title: "KaTeX Formula Test | EDA Visual Encyclopedia"
   - Include ALL 11 formula categories from the research test page pattern:
   ...
   - IMPORTANT: Since this is a `.astro` file (not MDX), KaTeX rendering via remark-math won't process raw `$...$` in the template. Instead, use `katex.renderToString()` in the frontmatter to pre-render each formula, then inject the HTML with `set:html`. Import `katex` from `'katex'` in the frontmatter script. This approach guarantees formula rendering works regardless of MDX pipeline.
   - Alternatively, create this as `test-formulas.mdx` in `src/pages/eda/` and use the MDX pipeline directly with `$...$` syntax. This tests the actual remark-math + rehype-katex pipeline that content pages will use. PREFER this approach as it validates the real pipeline.
...
5. Run build and verify the test page renders without errors. If `mathFlowInside` error occurs, execute the fallback: `npm install remark-math@5.1.1 rehype-katex@6.0.3` and rebuild.
  ...
  <verify>
    ...
    <manual>Build succeeds. dist/eda/test-formulas/index.html exists and contains multiple "katex" class references (pre-rendered formula HTML). Open in browser to verify all 11 formula categories render correctly with proper symbols, not raw LaTeX.</manual>
  </verify>
  <done>EDALayout.astro conditionally loads KaTeX CSS. EdaBreadcrumb renders EDA > Category > Technique navigation. TechniquePage.astro has all named slots. Test page at /eda/test-formulas/ renders 11 KaTeX formula categories (Greek, fractions, summations, integrals, roots, matrices, sub/superscripts, normal PDF, aligned equations, chi-squared, products) without build errors.</done>
</task>
