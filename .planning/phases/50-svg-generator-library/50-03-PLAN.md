...
must_haves:
  ...
  key_links:
    ...
    - from: "src/lib/eda/svg-generators/contour-plot.ts"
      ...
      via: "uses PALETTE CSS variable constants for contour band fills (no hardcoded hex colors)"
      pattern: "PALETTE\\.data"
...
```
This is the only new npm dependency for Phase 50 (~4KB, pure computation, marching squares algorithm for contour generation). Do NOT install d3-interpolate -- it is not needed.
...
2. Export `generateContourPlot(options): string`:
   ...
   **CRITICAL — Contour band coloring must use PALETTE CSS variables, NOT hardcoded hex colors (SVG-13 compliance):**
   - Do NOT use `d3-interpolate` or `interpolateRgbBasis`. Do NOT use hardcoded hex values like `#006d6d`, `#fffaf7`, `#c44b20`.
   - Instead, use a quantized opacity approach with PALETTE constants:
     - Normalize each contour level's threshold value to a 0-1 range: `t = (threshold - minVal) / (maxVal - minVal)`
     - For low t values (0-0.5): fill with `PALETTE.dataSecondary` at opacity `0.15 + (1 - t * 2) * 0.55` (stronger teal at lowest, fading toward midpoint)
     - For high t values (0.5-1.0): fill with `PALETTE.dataPrimary` at opacity `0.15 + (t - 0.5) * 2 * 0.55` (fading from midpoint, stronger orange at highest)
     - This produces a diverging colormap from teal (low) through transparent (mid) to orange (high) that automatically adapts to dark/light themes via CSS variables
   ...
   - Render filled contour bands as `<path>` with the PALETTE-based fill and computed opacity
   - Render contour lines (outlines) as `<path>` with PALETTE.textSecondary stroke, 0.5px
   - Add color legend: small rectangles at the right side. Use the same PALETTE fill + opacity scheme for legend swatches. Add threshold value labels using PALETTE.textSecondary
...
**Success criteria 5 requires normal, exponential, chi-square at minimum. This implements 6 distributions (normal, exponential, chi-square, uniform, t, gamma) per research recommendation.**
  ...
  <verify>
    <automated>cd /Users/patrykattc/work/git/PatrykQuantumNomad && npx tsx -e "
      ...
      // SVG-13 check: contour must NOT contain hardcoded hex fills
      const hexFillMatch = contour.match(/fill=\"#[0-9a-fA-F]{3,8}\"/g);
      if (hexFillMatch) throw new Error('Contour has hardcoded hex fill(s): ' + hexFillMatch.join(', ') + ' -- violates SVG-13');

      // SVG-13 check: contour must use CSS variable references for fills
      if (!contour.includes('var(--color')) throw new Error('Contour not using CSS variable references');
      ...
      console.log('PASS: contour (' + contour.length + '), 3 required dist curves + 3 additional verified, no hardcoded hex fills in contour');
    "</automated>
  </verify>
  <done>
    - d3-contour@^4.0.2 installed as npm dependency (d3-interpolate NOT needed)
    - responseSurface dataset added to datasets.ts (20x20 grid with polynomial response model)
    - generateContourPlot produces valid SVG with filled contour bands using PALETTE CSS variables at varying opacity levels (SVG-08, SVG-13 compliant)
    - Contour fills use quantized opacity with PALETTE.dataSecondary (low) and PALETTE.dataPrimary (high) -- no hardcoded hex colors
...
- 6-plot contains 6 sub-panels
  ...
  <done>
    ...
    - Phase 50 complete: all 15 SVG requirements satisfied via 13 generator functions + 2 cross-cutting requirements (SVG-12 viewBox, SVG-13 dark/light theme)
  </done>
</task>
...
4. Contour plot fills use PALETTE CSS variables with opacity -- no hardcoded hex `fill="#..."` attributes
5. 4-plot contains 4 `<g transform>` groups composing sub-generators
6. 6-plot contains 6 `<g transform>` groups composing sub-generators
7. Distribution curve works for normal, exponential, chi-square (success criteria 5)
8. `npm run build` succeeds without errors (no build regression)
</verification>
...
- Phase 50 success criteria all met: all 15 SVG requirements satisfied via 13 generator functions + 2 cross-cutting requirements (SVG-12 viewBox, SVG-13 dark/light theme), composite layouts, distribution curves for 3+ distributions
</success_criteria>
