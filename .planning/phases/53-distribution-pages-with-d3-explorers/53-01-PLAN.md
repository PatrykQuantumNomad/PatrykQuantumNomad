...
1. **cauchy**: PDF = 1/(pi*gamma*(1+((x-x0)/gamma)^2)), CDF = atan((x-x0)/gamma)/pi + 0.5
2. **weibull**: PDF = (k/lambda)*(x/lambda)^(k-1)*exp(-(x/lambda)^k) for x>=0, CDF = 1-exp(-(x/lambda)^k)
3. **lognormal**: PDF = (1/(x*sigma*sqrt(2*pi)))*exp(-(ln(x)-mu)^2/(2*sigma^2)) for x>0, CDF uses erf
4. **fatigue-life** (Birnbaum-Saunders): PDF via normalPDF transform, CDF via normalCDF of z(x) = (1/gamma)*(sqrt(x/mu)-sqrt(mu/x))
5. **double-exponential** (Laplace): PDF = (1/(2*beta))*exp(-|x-mu|/beta), CDF = piecewise
6. **power-normal**: PDF = c*normalPDF(x)*normalCDF(x)^(c-1), CDF = normalCDF(x)^c
7. **power-lognormal**: PDF = (c/(x*sigma))*normalPDF(z)*normalCDF(z)^(c-1) where z=(ln(x)-mu)/sigma, CDF = normalCDF(z)^c
8. **extreme-value** (Gumbel): PDF = (1/beta)*exp(-(z+exp(-z))) where z=(x-mu)/beta, CDF = exp(-exp(-z))
9. **beta**: PDF = (x^(a-1)*(1-x)^(b-1))/B(a,b) for x in [0,1], CDF = I_x(a,b) (regularizedBeta)
10. **f-distribution**: PDF via gammaFn and power formula, CDF = I(d1*x/(d1*x+d2); d1/2, d2/2) using regularizedBeta
11. **tukey-lambda**: Quantile function Q(F) = (F^lambda - (1-F)^lambda)/lambda. Special-case |lambda|<1e-10 as logistic: ln(F/(1-F)). PDF computed as f(x) = 1/Q'(F) where Q'(F) = F^(lambda-1) + (1-F)^(lambda-1). Generate curve by sweeping F from 0.001 to 0.999.
12. **binomial** (discrete): PMF = C(n,k)*p^k*(1-p)^(n-k) using lnBinomialCoeff in log-space. CDF = sum of PMF for k=0..floor(x).
13. **poisson** (discrete): PMF = exp(-lambda)*lambda^k/k! using lnGamma in log-space. CDF = sum of PMF for k=0..floor(x).
...
const defaults = {normal:{mu:0,sigma:1},uniform:{a:0,b:1},cauchy:{x0:0,gamma:1},'t-distribution':{nu:5},'f-distribution':{d1:5,d2:10},'chi-square':{k:4},exponential:{lambda:1},weibull:{k:1.5,lambda:1},lognormal:{mu:0,sigma:1},'fatigue-life':{gamma:0.5,mu:1},gamma:{alpha:2,beta:1},'double-exponential':{mu:0,beta:1},'power-normal':{c:2},'power-lognormal':{c:2,mu:0,sigma:1},'tukey-lambda':{lambda:0.14},'extreme-value':{mu:0,beta:1},beta:{alpha:2,beta:5},binomial:{n:20,p:0.5},poisson:{lambda:5}};
let pass = 0;
for (const id of ids) {
