...
- `distributionUrl` from routes
  ...
  <verify>
    <automated>cd /Users/patrykattc/work/git/PatrykQuantumNomad && npm run build 2>&1 | tail -5 && ls dist/eda/distributions/index.html && echo "--- Checking distribution links ---" && LINK_COUNT=$(grep -c 'href="/eda/distributions/' dist/eda/distributions/index.html) && echo "Distribution links: $LINK_COUNT" && echo "--- Checking SVG thumbnails ---" && SVG_COUNT=$(grep -c '<svg' dist/eda/distributions/index.html) && echo "SVG thumbnails: $SVG_COUNT" && echo "--- Checking D3 isolation: no d3 chunks on non-distribution EDA page ---" && if ls dist/eda/techniques/histogram/index.html >/dev/null 2>&1; then D3_REFS=$(grep -ci 'd3' dist/eda/techniques/histogram/index.html || true) && echo "D3 references on histogram page: $D3_REFS" && if echo "$D3_REFS" | grep -qE 'chunk.*d3|d3.*chunk|from.*d3-'; then echo "FAIL: D3 bundle leaking to non-distribution page"; exit 1; fi; else echo "SKIP: histogram page not built yet (OK for isolation check during visual verify)"; fi && echo "PASS: landing page and D3 isolation check complete"</automated>
    <manual>Landing page exists with 19 distribution links and 19 SVG thumbnails. Non-distribution EDA pages (e.g., /eda/techniques/histogram/) do not load D3 JS chunks.</manual>
  </verify>
  <done>Landing page at /eda/distributions/ renders with 19 distribution cards showing PDF shape thumbnails, each linking to the detail page. Page has breadcrumbs, JSON-LD, and proper SEO metadata. D3 bundle isolation confirmed (no D3 on non-distribution pages).</done>
</task>
