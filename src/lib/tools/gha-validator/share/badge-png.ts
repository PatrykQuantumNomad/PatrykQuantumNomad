 ...
 *
 * Builds an SVG badge with arc gauge + per-category breakdown bars,
 * rasterizes to a retina-aware PNG, and triggers a download.
 */

import type { GhaScoreResult, GhaCategoryScore } from '../types';

// ── Category metadata (mirrors GhaCategoryBreakdown.tsx) ─────────
...
function buildBadgeSvg(score: GhaScoreResult): string {
  ...
  const catBars = categories
    .map((cat: GhaCategoryScore, i: number) => {
      const y = 40 + i * 26;
  ...
  return [
    ...
    '<text x="135" y="24" fill="rgba(255,255,255,0.5)" font-size="10" font-family="system-ui, sans-serif">GHA Workflow Analysis</text>',
    catBars,
    `<text x="${W / 2}" y="${H - 10}" text-anchor="middle" fill="rgba(255,255,255,0.25)" font-size="8" font-family="system-ui, sans-serif">patrykgolabek.dev/tools/github-actions-workflow-validator</text>`,
    '</svg>',
  ].join('');
}
...
/**
 * Generate and download a PNG score badge with category breakdown.
 */
export async function downloadScoreBadge(score: GhaScoreResult): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    img.onload = () => {
      ...
      canvas.toBlob((pngBlob) => {
        ...
        a.download = `gha-score-${score.overall}-${score.grade}.png`;
