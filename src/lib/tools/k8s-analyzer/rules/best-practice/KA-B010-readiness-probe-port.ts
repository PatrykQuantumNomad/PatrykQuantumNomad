 ...
 *
 * When a readiness probe references a port that is not declared in the
 * container's ports array, the probe may fail to connect. This prevents
 * the pod from receiving traffic and can cause service outages.
 */
export const KAB010: K8sLintRule = {
  id: 'KA-B010',
  title: 'Readiness probe port not in container ports',
  severity: 'warning',
  category: 'best-practice',
  explanation:
    'The readiness probe references a port that is not declared in the container ports array. ' +
    'This may indicate a misconfiguration where the probe points to the wrong port or the ' +
    'container port declaration is missing. The pod will never become ready, blocking traffic.',
  fix: {
    description: 'Ensure the readiness probe port matches a declared container port',
    beforeCode:
      'containers:\n  - name: app\n    ports:\n      - containerPort: 8080\n    readinessProbe:\n      httpGet:\n        port: 9090',
    afterCode:
      'containers:\n  - name: app\n    ports:\n      - containerPort: 8080\n    readinessProbe:\n      httpGet:\n        port: 8080',
  ...
  check(ctx: K8sRuleContext): K8sRuleViolation[] {
    ...
    for (const resource of ctx.resources) {
      ...
      for (const { container, jsonPath, containerType } of containerSpecs) {
        ...
        if (!readinessProbe) continue;

        const probePort = getProbePort(readinessProbe);
        ...
        if (!declaredPorts.has(probePort)) {
          ...
          violations.push({
            ...
            line,
            column: col,
            message: `Container '${container.name}' in ${resource.kind} '${resource.name}' has a readiness probe on port ${probePort} which is not in the declared container ports.`,
          });
        }
      }
    }
