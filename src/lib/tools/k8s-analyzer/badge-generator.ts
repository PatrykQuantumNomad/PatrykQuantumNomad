import type { K8sScoreResult, K8sCategoryScore } from './types';
...
/** Build a standalone 400x200 SVG badge string for the given K8s score result */
export function buildK8sBadgeSvg(score: K8sScoreResult): string {
  ...
  const catBars = categories
    .map((cat: K8sCategoryScore, i: number) => {
      const y = 40 + i * 26;
  ...
  return [
    ...
    '<text x="135" y="24" fill="rgba(255,255,255,0.5)" font-size="10" font-family="system-ui, sans-serif">K8s Manifest Analysis</text>',
    catBars,
    `<text x="${W / 2}" y="${H - 10}" text-anchor="middle" fill="rgba(255,255,255,0.25)" font-size="8" font-family="system-ui, sans-serif">patrykgolabek.dev/tools/k8s-analyzer</text>`,
    '</svg>',
  ].join('');
}

/** Rasterize the K8s score badge SVG to a retina-aware PNG and trigger a browser download */
export async function downloadK8sBadgePng(score: K8sScoreResult): Promise<void> {
  const svgString = buildK8sBadgeSvg(score);
  ...
  return new Promise<void>((resolve, reject) => {
    img.onload = () => {
      ...
      canvas.toBlob((pngBlob) => {
        ...
        a.download = `k8s-score-${score.overall}-${score.grade}.png`;
