...
/** Build a standalone 400x200 SVG badge string for the given score result */
export function buildBadgeSvg(score: ScoreResult): string {
  ...
  const catBars = categories
    .map((cat: CategoryScore, i: number) => {
      const y = 40 + i * 26;
...
/** Rasterize the score badge SVG to a retina-aware PNG and trigger a browser download */
