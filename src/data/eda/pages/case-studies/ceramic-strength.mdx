...
DOE mean plots show the average response at each factor level, with a dashed line at the grand mean. Steeper lines indicate larger effects. These plots are the primary tool for ranking factor importance and determining optimal settings.

{/* DOE mean and SD plots will be wired in Plan 02 */}

### Interaction Plots

Interaction plots show whether the effect of one factor depends on the level of another factor. Non-parallel lines indicate interaction. The <InlineMath tex="X_1 \times X_3" /> interaction (table speed by wheel grit) is the most important interaction in both batches.

{/* Interaction plots will be wired in Plan 02 */}
