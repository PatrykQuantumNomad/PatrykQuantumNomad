...
The inconsistency in factor rankings across batches has direct engineering implications: no single set of optimal machining parameters applies to both batches. The batch effect was completely unexpected by the NIST investigators, demonstrating that unmeasured material differences between batches can dominate machining factor effects. This case study illustrates how graphical EDA techniques efficiently reveal multi-factor structure -- the [bihistogram](/eda/techniques/bihistogram/) exposes batch separation, [block plots](/eda/techniques/batch-block-plot/) confirm consistency across labs, and [DOE mean plots](/eda/techniques/doe-plots/) rank factor importance within each batch. A [multi-factor ANOVA](/eda/quantitative/multi-factor-anova/) with batch as a blocking factor is the recommended follow-up analysis.

## Conclusions

The ceramic strength data present a multi-faceted analysis challenge. The key findings, in order of importance, are:

1. **Dominant batch effect** — The [bihistogram](/eda/techniques/bihistogram/) and [block plots](/eda/techniques/batch-block-plot/) show Batch 1 specimens are on average <InlineMath tex="77.84" /> units stronger than Batch 2 (<InlineMath tex="T = 13.38" />, <InlineMath tex="p \ll 0.001" />), a difference that is consistent across all 8 labs and all 16 treatment combinations. This effect was completely unexpected by the scientific investigators.
...
The recommended analysis approach is a [multi-factor ANOVA](/eda/quantitative/multi-factor-anova/) that includes batch as a blocking factor, followed by examination of main effects and interactions among the primary machining factors -- analyzed separately by batch given the inconsistent factor rankings. This case study demonstrates how graphical techniques ([bihistograms](/eda/techniques/bihistogram/), [block plots](/eda/techniques/batch-block-plot/), [box plots](/eda/techniques/box-plot/) by group) efficiently reveal batch and lab effects that would be difficult to detect from summary statistics alone, and how [DOE mean plots, standard deviation plots, and interaction plots](/eda/techniques/doe-plots/) identify the key machining factors and their interdependencies within each batch.
