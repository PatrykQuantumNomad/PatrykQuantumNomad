...
A plugin can include any combination of the following component types. Each follows the same format it would use outside of a plugin -- the plugin system just provides the packaging and distribution wrapper.
...
Legacy flat Markdown command files for backward compatibility. These are the older `.claude/commands/` format -- single Markdown files without frontmatter-based configuration. New plugins should use `skills/` instead, which support richer configuration through frontmatter, automatic discovery in nested directories, and supporting files.
...
Hook configurations that activate when the plugin is enabled. The `hooks.json` file uses the same schema as the `hooks` object in `settings.json` -- the same event names, matcher patterns, handler types, and `if` field syntax described in [Hooks & Lifecycle Automation](/guides/claude-code/hooks/).
...
Plugin hooks merge with hooks from other sources (user, project, managed settings) -- they don't override them. See [Hooks & Lifecycle Automation](/guides/claude-code/hooks/) for the full merging behavior.
...
- **description** -- Human-readable prompt shown to the user at enable time
- **required** -- Whether the user must provide a value (default: false)
- **secret** -- Whether the value is stored in the system keychain (default: false)
- **default** -- Default value used if the user doesn't provide one
