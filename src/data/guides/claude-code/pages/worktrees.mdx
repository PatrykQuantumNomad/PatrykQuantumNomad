...
description: "Parallelize development across isolated git worktree branches using built-in and custom subagents with AGENT.md persona definitions, persistent memory, initialPrompt auto-submit, and sparse-checkout for monorepos."
order: 8
slug: "worktrees"
lastVerified: 2026-04-12
updatedDate: 2026-04-12
keywords: ["claude code subagents", "git worktrees", "AGENT.md", "parallel development", "delegation", "custom agents", "worktree isolation", "sparsePaths", "initialPrompt", "agent memory", "background tasks"]
---
...
# Or invoke a specific agent by name
"@security-reviewer check the auth module for vulnerabilities"
→ Invokes the named security-reviewer subagent directly`}
  lang="text"
  title="Invoking built in subagents"
  caption="Claude Code chooses the right subagent based on your request, or you can target one by name with @agent-name."
/>
...
<CodeBlock
  code={`---
name: code-reviewer
description: Reviews staged changes for security and quality issues
tools: Read, Grep, Glob, Bash
...
Output a structured report with severity levels and specific file references.`}
  lang="markdown"
  title=".claude/agents/code-reviewer.md"
  caption="initialPrompt auto-submits when this agent runs as the main session agent via --agent code-reviewer."
/>
...
<CodeBlock
  code={`claude --agents '[{"name": "reviewer", "description": "Reviews code for issues"}, {"name": "tester", "description": "Writes and runs tests"}]'`}
  lang="bash"
  title="Defining subagents inline with --agents"
  caption="Session-only agents defined as JSON. Useful for automation scripts and quick experiments."
/>
...
**Plugin** agents are installed from plugin marketplaces and managed by Claude Code's plugin system. For details on creating and distributing plugins that include agents, see [Plugins](/guides/claude-code/plugins/).
...
By default, worktrees are cleaned up automatically when the session ends. The **ExitWorktree** tool handles this cleanup, removing the worktree branch once the session finishes and there are no uncommitted changes. You can configure this behavior if you'd rather have worktrees persist.
...
<CodeBlock
  ...
  caption="Worktrees only check out the specified directories instead of the full repo."
/>
...
<CodeBlock
  code={`# User-scoped memory (global, all projects)
~/.claude/agent-memory/
  code-reviewer/
    MEMORY.md                    # Main entrypoint, read on startup
    coding-standards.md          # Topic file: learned style preferences
    common-patterns.md           # Topic file: recurring code patterns

# Project-scoped memory (per-repository)
my-project/.claude/agent-memory/
  security-reviewer/
    MEMORY.md                    # Project-specific memory
    vulnerability-history.md     # Topic file: past findings`}
  lang="text"
  title="Agent memory directory structure"
  caption="MEMORY.md is the entrypoint. Topic files organize knowledge by subject."
/>
...
<CodeBlock
  code={`export CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1`}
  lang="bash"
  title="Disabling background tasks"
  caption="When set, all subagents run in the foreground regardless of their background setting."
/>
...
**Library** shows all available agents from every source: built in agents, project agents from `.claude/agents/`, user agents from `~/.claude/agents/`, and agents provided by installed [plugins](/guides/claude-code/plugins/). From the Library tab you can:
...
<CodeBlock
  ...
  caption="The Running tab shows active agents. The Library tab shows all available agents across all scopes."
/>
...
**auto**: A classifier-based mode where safe actions proceed automatically and destructive or suspicious actions are blocked. This provides intelligent permission management without manual approvals. See the [Security chapter](/guides/claude-code/security/) for full details on auto mode behavior.
...
auto             → Classifier-based safe/block decisions
...
For production workflows and CI/CD pipelines, you can create and manage agents programmatically using the [Agent SDK](/guides/claude-code/agent-sdk/). The SDK exposes `AgentDefinition` objects for defining agents in Python or TypeScript, with the same capabilities as AGENT.md files but controlled through code. This is particularly useful when agent configurations need to be dynamic or generated at runtime.
...
- **Use sparsePaths in monorepos.** If your subagents only need a subset of the repository, configure `worktree.sparsePaths` to keep worktrees lean and creation fast.
