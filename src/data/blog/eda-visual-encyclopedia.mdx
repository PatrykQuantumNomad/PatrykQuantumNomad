...
coverImage: "/images/eda-visual-encyclopedia-cover.svg"
draft: false
---
...
The largest section covers [29 graphical techniques](/eda/#techniques) ranging from fundamental tools like [histograms](/eda/techniques/histogram/), [scatter plots](/eda/techniques/scatter-plot/), and [box plots](/eda/techniques/box-plot/) to specialized diagnostic plots like [autocorrelation plots](/eda/techniques/autocorrelation-plot/), [normal probability plots](/eda/techniques/normal-probability-plot/), and [Weibull plots](/eda/techniques/weibull-plot/).

Each technique page includes an interactive SVG visualization generated from realistic sample data, a clear explanation of what the plot reveals, interpretation guidelines, and variant visualizations where applicable. The [scatter plot page](/eda/techniques/scatter-plot/) shows both the basic scatter and a version with a confidence band, while the [box plot page](/eda/techniques/box-plot/) demonstrates single-group and multi-group comparisons.

A few specialized techniques deserve particular attention:

- The [PPCC plot](/eda/techniques/ppcc-plot/) (Probability Plot Correlation Coefficient) is a powerful but underused tool for identifying the best-fitting distribution family. It sweeps across shape parameters, and the peak correlation indicates the closest distributional match.

- The [star plot](/eda/techniques/star-plot/) provides a multivariate visualization that maps each variable to a spoke of a radial chart, making it possible to compare multiple observations across many dimensions at once.

- The [spectral plot](/eda/techniques/spectral-plot/) uses power spectral density estimation to detect hidden periodicities in time series data. This is essential for manufacturing quality control and signal processing.
...
The [probability distributions section](/eda/#distributions) is where the encyclopedia becomes genuinely interactive. Each of the 19 distributions, from the ubiquitous [normal distribution](/eda/distributions/normal/) to specialized ones like the [Tukey-Lambda](/eda/distributions/tukey-lambda/) and [fatigue-life (Birnbaum-Saunders)](/eda/distributions/fatigue-life/), includes a client-side distribution explorer built with D3.
...
The mathematical formulas for each distribution are rendered with KaTeX directly in the page. The [related distributions page](/eda/reference/related-distributions/) maps the connections between all 19 distributions: which are special cases of others, which converge in the limit, and which arise as transformations.

### Case Studies: 9 Real-World Analyses

The [case studies section](/eda/#case-studies) walks through 9 complete EDA analyses on real engineering datasets from the NIST archive. These are not toy examples. The [ceramic strength study](/eda/case-studies/ceramic-strength/) analyzes a designed experiment to uncover an unexpected batch effect in silicon nitride specimens. The [heat flow meter study](/eda/case-studies/heat-flow-meter/) examines calibration data to confirm a well-behaved measurement process. The [fatigue life study](/eda/case-studies/fatigue-life/) compares four candidate probability distributions for aluminum alloy failure-time data.

Each case study follows the NIST methodology: state the questions, examine the data graphically, apply quantitative tests, and draw conclusions. The [beam deflections study](/eda/case-studies/beam-deflections/) is a great starting point because it shows how the autocorrelation plot and spectral analysis detect hidden periodic structure that summary statistics completely miss.
...
If you are new to EDA, I recommend starting with the [foundations section](/eda/foundations/what-is-eda/) to understand the philosophy, then working through the [4-plot technique](/eda/foundations/the-4-plot/) which gives you a universal starting point for any dataset. From there, the [beam deflections case study](/eda/case-studies/beam-deflections/) demonstrates how autocorrelation and spectral analysis uncover periodic structure that simple summary statistics miss entirely.

For experienced practitioners, the [reference tables](/eda/reference/techniques-by-category/) provide quick lookups organized by analysis goal, and the [distribution explorer](/eda/distributions/) is useful for comparing distributional fits and understanding parameter sensitivity.
