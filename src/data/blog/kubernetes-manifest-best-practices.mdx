...
description: "Production-tested guide to writing secure Kubernetes manifests. 67 rules across security, reliability, RBAC, and cross-resource validation with fix examples."
...
1. **Never run containers in privileged mode** ([KA-C001](/tools/k8s-analyzer/rules/ka-c001/)). It disables all container isolation.
2. **Set `allowPrivilegeEscalation: false`** ([KA-C002](/tools/k8s-analyzer/rules/ka-c002/)). The default is permissive, which means you must opt out explicitly.
3. **Enforce `runAsNonRoot: true`** ([KA-C003](/tools/k8s-analyzer/rules/ka-c003/)). Containers run as root by default unless you prevent it.
4. **Define liveness and readiness probes** ([KA-R001](/tools/k8s-analyzer/rules/ka-r001/), [KA-R002](/tools/k8s-analyzer/rules/ka-r002/)). Without probes, Kubernetes cannot detect unhealthy pods.
5. **Set CPU and memory requests and limits** ([KA-B001](/tools/k8s-analyzer/rules/ka-b001/) through [KA-B004](/tools/k8s-analyzer/rules/ka-b004/)). A pod without limits can starve every other pod on the node.
6. **Pin image tags to specific versions** ([KA-R009](/tools/k8s-analyzer/rules/ka-r009/)). The `:latest` tag is a moving target that breaks reproducibility.
7. **Deploy multiple replicas with anti-affinity** ([KA-R004](/tools/k8s-analyzer/rules/ka-r004/)). A single replica is a single point of failure.
8. **Drop all capabilities and add back selectively** ([KA-C011](/tools/k8s-analyzer/rules/ka-c011/)). Containers inherit capabilities you probably do not need.
9. **Never use wildcard RBAC permissions** ([KA-A001](/tools/k8s-analyzer/rules/ka-a001/)). Scope every Role to specific resources and verbs.
10. **Always specify a namespace** ([KA-B006](/tools/k8s-analyzer/rules/ka-b006/)). Omitting it deploys to whatever namespace `kubectl` is currently configured for.

You can validate all ten automatically with the free [Kubernetes Manifest Analyzer](/tools/k8s-analyzer/), an online YAML validator and linter that checks all 67 rules in the browser.
...
Rule [KA-R001](/tools/k8s-analyzer/rules/ka-r001/) flags containers that define no probes at all. Missing probes are the single most common reliability issue in the Kubernetes manifests I review. The impact is invisible during normal operation and catastrophic during failures, which is exactly the combination that causes long, painful outages.
...
- <a href="https://kubernetes.io/docs/concepts/security/pod-security-standards/" target="_blank" rel="noopener noreferrer">Kubernetes Pod Security Standards</a> define the official Baseline and Restricted security profiles.
- <a href="https://www.cisecurity.org/benchmark/kubernetes" target="_blank" rel="noopener noreferrer">CIS Kubernetes Benchmark</a> provides audit-ready controls for cluster and workload security (Sections 5.1 and 5.2 map directly to our RBAC and security rules).
- <a href="https://csrc.nist.gov/pubs/sp/800/190/final" target="_blank" rel="noopener noreferrer">NIST SP 800-190: Application Container Security Guide</a> covers container image, runtime, and orchestrator hardening.
- <a href="https://cheatsheetseries.owasp.org/cheatsheets/Kubernetes_Security_Cheat_Sheet.html" target="_blank" rel="noopener noreferrer">OWASP Kubernetes Security Cheat Sheet</a> provides a concise reference for common Kubernetes security patterns.

## Start Analyzing

If you have read this far, you know what good Kubernetes manifests look like. Now find out what yours actually score.

The [Kubernetes Manifest Analyzer](/tools/k8s-analyzer/) is a free online Kubernetes YAML validator and static analysis tool. Paste your manifests, read the results, and follow the links to individual rule documentation pages for detailed fix guidance. Every rule page includes explanations of why the rule exists, what production issues it prevents, PSS and CIS mappings where applicable, and related rules. The tool runs entirely in your browser, so your manifests never leave your device.
...
<Callout>
**Containerizing your applications?** Check your Dockerfiles with the [Dockerfile Analyzer](/tools/dockerfile-analyzer/), which covers 40 rules for security, efficiency, and maintainability. Orchestrating with Compose? Validate your docker-compose.yml with the [Docker Compose Validator](/tools/compose-validator/), which covers 52 rules for schema, security, semantic, and best-practice analysis.
</Callout>
