...
Rule [GA-L005](/tools/gha-validator/rules/ga-l005/) detects references to context properties that don't exist, such as `${{ github.events.pull_request }}` (the correct property is `github.event`, singular). These typos produce empty values at runtime with no error, leading to subtle bugs that are difficult to trace.
