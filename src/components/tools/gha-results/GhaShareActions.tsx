...
import {
  encodeGhaState,
  isUrlTooLong,
} from '../../../lib/tools/gha-validator/share/url-state';
import { GhaPromptGenerator } from './GhaPromptGenerator';

export function GhaShareActions() {
  ...
  const handleDownloadBadge = async () => {
    setDownloading(true);
    try {
      await downloadScoreBadge(result.score);
    } catch (err) {
  ...
  const handleShare = async () => {
    ...
    const url = window.location.origin + window.location.pathname + encodeGhaState(content);

    if (isUrlTooLong(encodeGhaState(content))) {
      setUrlWarning('URL is long and may be truncated on some platforms');
    } else {
      setUrlWarning(null);
    }
